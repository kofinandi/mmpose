...
- $\mathbf{R_{past}} = [R_{t-N}, \dots, R_{t-1}]^T$: Vector of their corresponding **measurement variances**.
- $z_t$: Measurement at time $t$.
- $R_t$: Measurement variance at time $t$.
...
If a measurement $z_t$ with variance $R_t$ is recorded at time $t$, we perform a Bayesian update fusing the GP prediction and the measurement.

**Compute the Kalman Gain:**
$$K_t = \frac{\Sigma_{t|t-1}}{\Sigma_{t|t-1} + R_t}$$

**Update the Mean (Posterior Output):**
$$\mu_{t|t} = \mu_{t|t-1} + K_t (z_t - \mu_{t|t-1})$$
