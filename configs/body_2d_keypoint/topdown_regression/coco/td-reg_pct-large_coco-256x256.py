...
# PCT-large on COCO 256×256 – inference / evaluation only.
...
train_cfg = None
# Checkpoint: external/PCT/weights/pct/swin_large.pth
#
# Run:
#   python tools/test.py \
#       configs/body_2d_keypoint/topdown_regression/coco/td-reg_pct-large_coco-256x256.py \
#       external/PCT/weights/pct/swin_large.pth
...
model = dict(
    ...
    pct_model_cfg=dict(
        # Swin-Large backbone (embed_dim=192, out_indices=(3,) → 1536-ch feature)
        backbone=dict(
        ...
        keypoint_head=dict(
            ...
            tokenizer=dict(
                guide_ratio=0.5,
                ckpt='external/PCT/weights/pct/swin_large.pth',
                encoder=dict(
                    drop_rate=0.2,
                    num_blocks=4,
                    hidden_dim=512,
