...
# AlphaPose -- FastPose with deformable convs (ResNet-50), COCO-17, 256x192, with
...
#
# Mirrors the author config `configs/coco/resnet/256x192_res50_lr1e-3_2x-dcn.yaml`
...
# STAGE_WITH_DCN=[false, true, true, true], MODULATED=false.
# NOTE: this variant needs the compiled `deform_conv_cuda` extension,
# i.e. the `pip install -e external/AlphaPose` build must have succeeded
# (it is the only config here that does).
#
...
#
# Checkpoints:
#   pose : data/models/alphapose/fast_dcn_res50_256x192.pth
#          (author-released, "Fast Pose (DCN)" in external/AlphaPose/docs/MODEL_ZOO.md,
#           Google Drive id 1zUz9YIk6eALCbZrukxD7kQ554nhi1pVv)
...
#     configs/body_2d_keypoint/alphapose/alphapose_fastpose-dcn-res50_coco-256x192.py \
#     data/models/alphapose/fast_dcn_res50_256x192.pth \
...
model = dict(
    ...
    alphapose_cfg='configs/coco/resnet/256x192_res50_lr1e-3_2x-dcn.yaml',
    tracker=dict(
